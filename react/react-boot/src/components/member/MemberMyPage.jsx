...
function MemberMyPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append("pass", member.pass);
        if (picture) formData.append("profileFile", picture);

        const response = await fetch("http://localhost:8080/member/myPagePro", {
            method: "POST",
            body: formData
        });
        if (response.ok) {
    ...
    const handleDelete = async () => {
        ...
        const response = await fetch("http://localhost:8080/member/deletePro?id=" + cookies.id, {
            method: "DELETE"
        });
        if (response.ok) {
            alert("탈퇴되었습니다.");
        ...
        <div className="container mt-4" style={{maxWidth: "500px"}}>
            <div className="card shadow-sm">
                    ...
                    <form id="mypage-form" onSubmit={handleSubmit}>
                        ...
                        <div className="form-group text-center">
                            {preview ? (
                                ...
                                <img src={"http://localhost:8080/img/member/" + member.picture}
                ...
                <div className="card-footer bg-white d-flex justify-content-between align-items-center">
                    ...
                    <button className="btn btn-dark btn-sm px-4" form="mypage-form">
                        수정완료
                    </button>
                </div>
